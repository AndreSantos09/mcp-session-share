#!/usr/bin/env python3
"""
Poller headless do session-share — "cenário 3" (listener sem LLM no loop):
opção primária da skill session-listen deste plugin, no lugar do sub-agente
Claude em loop de session_poll.

Faz o long-poll de session_poll direto via streamable-http do MCP (pacote
`mcp` — ver requirements.txt ao lado, `pip install -r requirements.txt` uma
vez por máquina), usando o room_id/participant_id de uma sessão já
conectada. NÃO decide nada sobre o conteúdo — só repassa. Fica em silêncio
total em poll vazio; imprime UMA linha em stdout apenas quando chega
mensagem real (origin="participant"), um evento de sistema
(origin="system") ou a room fecha. É esse stdout que a tool Monitor do
Claude Code transforma em notificação — cada linha, um evento; nada
impresso, nenhuma notificação, nenhum turno de LLM gasto.
...
Auth: se o servidor session-share exigir token (AUTH_ENABLED=true),
exporte SESSION_SHARE_TOKEN no ambiente antes de rodar — o script manda
como Bearer automaticamente. Sem a env var, nenhum header de auth é
enviado (funciona contra um deploy com AUTH_ENABLED=false).

Uso (pensado pra ser o `command` de um Monitor):
    python3 session_poll_watch.py \\
...
def _extract_result(call_result) -> dict:
    ...
    primeiro content block (mesma lógica do hook remind-listener.py deste
    plugin)."""
